...
from utils.params_utils import parse_params_file, compute_interaction_matrix,\
                               prepare_interaction_matrix_params, generate_im_filename
...
yaml_file = os.path.join(specula_repo_path, "main", "scao", "params_morfeo_full2.yml")
...
paramsAll['main']['root_dir'] = os.path.join(specula_repo_path, "main", "scao","calib","MCAO")
print(f"Absolute path of root directory: {paramsAll['main']['root_dir']}")

# for cycle on all WFS, 3, and DM, 3
for ii in range(3):
    for jj in range(3):
        
        print(f"Processing WFS {ii+1} and DM {jj+1}")
        ...
        params = prepare_interaction_matrix_params(paramsAll,wfs_type='ngs',wfs_index=ii+1,dm_index=jj+1)
        ...
        print(f"--> Parameters for interaction matrix computation: {params}")

        # Calculate the interaction matrix
        im = compute_interaction_matrix(params, verbose=True, display=False)
