...
from synim.synim import (
    projection_matrix,
    projection_matrices_multi_base,
    rotshiftzoom_array,
...
class TestProjection(unittest.TestCase):
    ...
    def test_multi_base_same_transforms(self):
        """Test projection_matrices_multi_base with same transformations"""

        # Create 3 different basis sets (all with same transformations)
        base_configs = []
        for i in range(3):
            base_ifunc = IFunc(
                type_str='zern',
                npixels=self.pixel_pupil,
                nmodes=20 + i*5,  # Different number of modes
                obsratio=0.0,
                diaratio=1.0,
                target_device_idx=-1
            )
            base_array = base_ifunc.ifunc_2d_to_3d(normalize=True)

            base_configs.append({
                'base_inv_array': base_array,
                'rotation': 10.0,  # Same for all
                'translation': (0.2, 0.1),  # Same for all
                'magnification': (1.0, 1.0),  # Same for all
                'name': f'basis_{i}'
            })
        ...
        # Compute with multi-base method
        pm_multi_dict, info = projection_matrices_multi_base(
            self.pup_diam_m, self.pup_mask,
            self.dm_array, self.dm_mask,
            self.dm_height, self.dm_rotation,
            base_configs, gs_pol_coo, gs_height,
            verbose=True, specula_convention=True
        )

        print(f"\nMulti-base workflow: {info['workflow']}")
        # CORRECTED: Should use combined workflow when both DM and base have transforms
        self.assertEqual(info['workflow'], 'combined',
                        "Should use combined workflow when both DM and base have transforms")
        ...
        for base_config in base_configs:
            ...
            np.testing.assert_allclose(
                ...
                err_msg=f"Single and multi-base methods differ for {base_config['name']}"
            )

    def test_multi_base_same_transforms_separated(self):
        """Test projection_matrices_multi_base with same transforms - separated workflow"""

        # Create 3 different basis sets (NO transformations)
        base_configs = []
        for i in range(3):
            base_ifunc = IFunc(
                type_str='zern',
            ...
            base_configs.append({
                ...
                'magnification': (1.0, 1.0),  # NO magnification
                'name': f'basis_{i}'
            })

        # DM has transformations, but base doesn't
        gs_pol_coo = (5.0, 30.0)
        gs_height = np.inf
        ...
        print(f"\nMulti-base workflow: {info['workflow']}")
        # Should use separated workflow (DM transforms, but NO base transforms)
        self.assertEqual(info['workflow'], 'separated',
    ...
    def test_multi_base_different_transforms(self):
        """Test projection_matrices_multi_base with different transformations"""

        # Create 3 basis sets with different transformations
        base_configs = []
        rotations = [0.0, 15.0, 30.0]
        translations = [(0.0, 0.0), (0.5, 0.0), (0.0, 0.5)]
        ...
        for i in range(3):
            ...
            base_configs.append({
                'base_inv_array': base_array,
                'rotation': rotations[i],  # Different
                'translation': translations[i],  # Different
        ...
        print(f"\nMulti-base workflow: {info['workflow']}")
        self.assertEqual(info['workflow'], 'combined',
                        "Should use combined workflow for different transforms")
